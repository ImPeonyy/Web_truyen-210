    ...
    <div class="avatar__container">
        <div class="box__1">
            <div class="img-container">
                <h2 id="title-avatar">Avatar</h2>
                <div class="avatar">
                <?php
                require'connect.php';
                $sql = "SELECT * FROM avatar_user";
                $result = mysqli_query($conn, $sql);
                while ($row = mysqli_fetch_assoc($result)) 
                {
                    $image_path = $row['image_path'];
                    echo "<img id='avatar-img' src='$image_path' alt='Avatar'>";
                }
                ?>
                </div>
            </div>
            ...
            <div class="follower-title-box">
                <h2 id="title-follower">Người theo dõi</h2>
                    <div class="follower">
                    <?php
                    require'connect.php';
                    $sql = "SELECT * FROM nguoi_theo_doi";
                    $result = mysqli_query($conn, $sql);
                    ...
                    if (mysqli_num_rows($result) > 0) 
                    {
                        echo "<ul>";
                            while($row = mysqli_fetch_assoc($result)) 
                            {
                                echo "<li>" . $row["ten_nguoi_theo_doi"] . "</li>";
                            }
                        echo "</ul>";
                    } 
                    else 
                    {
                        echo "Không có người theo dõi nào!";
                    }
                    mysqli_close($conn);
                    ?>
                    </div>  
        ...
        <div class="box__2">
            ...
            <div class="user-info">
                <h2 id="title-user-info">Giới thiệu bản thân</h2>
                <form class="form">
                    <input type="text" id="user-info-input" placeholder="Vui lòng nhập thông tin bản thân (;">
                </form>
            </div>


            <div class="user-info-achievement">
                <h2  id="title-info-achievement">Thành tựu</h2>

                <div class="user-interact"> 
                <?php
                require'connect.php';
                $sql = "SELECT truyen_da_dang, truyen_theo_doi, luot_tuong_tac FROM thanh_tuu";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) 
                { 
	                echo "<table><tr><th>Truyện đã đăng</th><th>Truyện theo dõi</th><th>Lượt tương tác</th></tr>";
		            while($row = $result->fetch_assoc()) 
		            { 
			            echo "<tr><td class='col'>".$row["truyen_da_dang"]."</td><td class='col'>".$row["truyen_theo_doi"]."</td><td class='col'>".$row["luot_tuong_tac"]."</td></tr>";
                    }  
                    echo "</table>";
		        } 
	            else 
        ...
        <button id="popupBtn">Liên hệ với admin</button>
        <div id="popup" class="popup">
            <div class="popup-content">
            <span class="close">&times;</span>
            <h2>Nhắn tin cho admin</h2>
            <form id="popup-form">
